varying vec2 vUv;
		...
		void main() {
			...
			vec2 pos = vec2(0.5);
				pos.x+=sin(time*1.0+sin(offsetm*100.0+time*3.0)+sin(offsetm*100.0+time*1.0)*3.0)*0.3;
				pos.y+=cos(time*1.0+sin(offsetm*100.0+time*3.0)+sin(offsetm*100.0+time*1.0)*3.0)*0.3;
				
				puv-=pos;
				puv*=scale(vec2(.99+offsetm*0.025));
				puv+=pos;
				
				vec4 fb = texture2D(feedback,puv)*1.00;
				vec3 fin = vec3(e)*0.002 +fb.rgb;
